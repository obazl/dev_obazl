...
let test4 test_ctxt = assert_equal 1   (Main.alpha)

(* by default nslib naming is not transitive, so you can also access the
   nslib submodule directly *)
let test5 test_ctxt = assert_equal 255 (Color.Red.v)
let test6 test_ctxt = assert_equal 127 (Color.Green.v)
let test7 test_ctxt = assert_equal 80  (Color.Blue.v)
